...
async fn test_scan_zed_providers_single_account() {
    ...
    fs::write(&auth_file, serde_json::to_string_pretty(&auth_data).unwrap()).unwrap();
...
async fn test_scan_zed_providers_multiple_accounts() {
    ...
    fs::write(&auth_file1, serde_json::to_string_pretty(&auth_data1).unwrap()).unwrap();
    ...
    fs::write(&auth_file2, serde_json::to_string_pretty(&auth_data2).unwrap()).unwrap();

    let providers = scan_zed_providers(&store).await.unwrap();
    assert_eq!(providers.len(), 2);

    // Verify both providers are present
    let user_ids: Vec<String> = providers.iter().map(|(_, p)| p.user_id().to_string()).collect();
...
async fn test_scan_zed_providers_ignores_non_zed() {
    ...
    fs::write(&kiro_file, serde_json::to_string_pretty(&kiro_data).unwrap()).unwrap();
...
async fn test_scan_zed_providers_skips_invalid() {
    ...
    fs::write(&valid_file, serde_json::to_string_pretty(&valid_data).unwrap()).unwrap();
    ...
    fs::write(&invalid_file, serde_json::to_string_pretty(&invalid_data).unwrap()).unwrap();
